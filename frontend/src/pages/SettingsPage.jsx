...
function SettingsPage() {
  ...
  useEffect(() => {
    ...
    settingsApi
      ...
      .catch((err) => {
        console.warn("Could not fetch live settings, using fallback:", err);
      });
  ...
  const handleSave = async () => {
    ...
    try {
      await settingsApi.updateSettings(settings).catch(() => null);
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      alert("Export failed or backend unavailable");
    }
  };
  ...
  const handleReset = async () => {
    if (window.confirm("Are you sure you want to reset all data back to demo defaults?")) {
      try {
        ...
        alert("Reset failed or backend unavailable");
      }
    }
  };
