...
function IncomePage() {
  const [incomeList, setIncomeList] = useState(incomeTransactions);
  ...
  useEffect(() => {
    ...
    transactionsApi
      ...
      .then((res) => {
        if (isMounted && res && res.transactions && res.transactions.length > 0) {
          const iconMap = {
          ...
          const formatted = res.transactions.map((tx) => ({
            id: tx.id,
            source: tx.title || tx.description || "Income",
            ...
            type: tx.recurrence || "One-time",
            icon: iconMap[tx.category] || Wallet,
            accent: accentMap[tx.category] || "cyan",
          }));
          setIncomeList(formatted);
        }
      })
      .catch((err) => {
        console.warn("Could not fetch live income, using fallback:", err);
      });
  ...
  const filteredTransactions = useMemo(() => {
    return incomeList.filter((transaction) => {
      const matchesSearch =
        transaction.source
          .toLowerCase()
